## Hi there 👋

# 🕊️ Mads Barclay Paaskesen (AKA Mads Paaskeesn) / mp-it.dk

Hi, I’m Mads — a full-stack developer with 30+ years of experience building real systems, tools, and products.

---

## 🚀 What I build

- 🎬 **[Flixlar](https://flixlar.com)**  

  Desktop caption editor for fast, practical video workflows (Electron, ffmpeg, SRT)

- 📊 **[MySiteChart](https://mysitechart.com)**  

  Lightweight traffic & counter service (reborn from an early 2005 project)

- 🧠 **[Sacred AI](https://sacred-ai.com)**  

  Experimental emotional intelligence system — exploring human-centered AI

---

## 🧰 Tech & systems

**Frontend**
React · Vue · Nuxt · Tailwind

**Backend**
Node · PHP · .NET (legacy) · REST APIs

**Desktop**
Electron · native integrations

**Data**
PostgreSQL · MySQL · Redis · real-time systems

**Media & tooling**
ffmpeg · SRT processing · video workflows

**Infrastructure**
Linux · Docker (light) · Cloud / CI pipelines

I’ve worked across stacks for decades — I choose tools based on the problem, not trends.

---

## 🧠 How I work

- Fast over perfect  

- Simple beats complex  

- Ship → learn → improve  

- No unnecessary abstraction

> I build tools that solve real problems — not just demos.

---

## 🌍 About

🇩🇰 Denmark · Open to freelance, consulting, or interesting collaborations

---

## 🔗 Links

🌿 [mp-it.dk](https://mp-it.dk)  

💼 [LinkedIn](https://linkedin.com/in/madspaaskesen)  

🌙 [sacred-ai.com](https://sacred-ai.com)

---

⭐ If something here resonates — feel free to star, fork, or reach out.

[![mySiteChart](https://badge.mysitechart.com/badge/github/madspaaskesen)](https://mysitechart.com/In?github=madspaaskesen)

<!--
Focus: simple, fast systems that actually work.

- 🌐 Creator of [mysitechart.com](https://mysitechart.com) – a reborn link & traffic network (originally 2005)
- 🧘‍♂️ Building [Sacred AI](https://sacred-ai.com) – emotional intelligence for the soul, not just the screen
- 🧪 Explorer of lightweight, meaningful tools — from counters to conflict comforters
- 🇩🇰 Based in Denmark · Open to remote freelance & collaboration
--><!--
![MySiteChart badge](https://api.mysitechart.com/badge/madspaaskesen/count.svg)
--><!--
> “Build something that whispers truth — even in silence.”

---

🪄 If something here makes you smile, think, or feel — a ⭐ star is always appreciated.

🔧 Curious minds are welcome — feel free to fork, remix, or open an issue.

🌊 I believe in open tools that gently ripple through others. If you build on something I've made — I’d *love* to hear about it.

---

🌿 [mp-it.dk](https://mp-it.dk) | 🧠 [FaceBook](https://www.facebook.com/paaskesen.it) - [LinkedIn](https://linkedin.com/in/madspaaskesen) | 🌙 [sacred-ai.com](https://sacred-ai.com)


--><!--
**madspaaskesen/madspaaskesen** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
